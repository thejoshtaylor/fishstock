...
void Util::printBoard(Board *board)
{
    ...
    for (int i = 7; i >= 0; i--)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            char c = board->getPieceLetter(board->getPiece((Board::Position){(uint8_t)i, (uint8_t)j}));

            // doesnt matter what the white var is on a blank tile, but I can only call isWhitePiece on an occupied tile
            bool white = false;
            if (board->getPiece((Board::Position){(uint8_t)i, (uint8_t)j}) != Board::PieceType::EMPTY)
            {
                white = board->isWhitePiece(board->getPiece((Board::Position){(uint8_t)i, (uint8_t)j}));
            }
